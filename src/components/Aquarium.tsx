...
const CRYPTO_NAMES = [
  "Satoshi", "Nakamoto", "Hal Finney", "Hodl", "Lambo",
  "Moonfish", "Whale Jr.", "Shrimp", "Diamond Fins", "Paper Tail",
  "Block", "Hash", "Nonce", "Merkle", "Genesis",
  "DeFi Dan", "Rug Pull Rick", "Pump", "Dump", "WAGMI",
  "gm", "ser", "fren", "Anon", "AlphaFin",
  "Vitalik Jr.", "CZ's Goldfish", "Elon's Koi", "Doge Fish", "Pepe Guppy"
];

const FISH_COLORS = [
  "#ff6b35", "#ff1744", "#d500f9", "#651fff", "#2979ff",
  "#00e5ff", "#00e676", "#ffea00", "#ff9100", "#f50057",
  "#7c4dff", "#18ffff", "#76ff03", "#ffd740", "#ff6e40",
];

const SPECIES = [
  "Clownfish", "Angelfish", "Pufferfish", "Beta", "Goldfish",
  "Neon Tetra", "Guppy", "Swordtail", "Koi", "Tang"
];
...
function createFish(overrides?: Partial<FishData>): FishData {
  ...
  return {
    ...
    y: 80 + Math.random() * (typeof window !== "undefined" ? window.innerHeight - 250 : 400),
...
export default function Aquarium() {
    ...
    const animate = (now: number) => {
      ...
      setFish((prev) =>
        prev.map((f) => {
          ...
          if (x > w - 80) { x = w - 80; vx = -Math.abs(vx) * 0.8; }
          if (y < 50) { y = 50; vy = Math.abs(vy) * 0.8; }
          if (y > h - 120) { y = h - 120; vy = -Math.abs(vy) * 0.8; }
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      if ((e.target as HTMLElement).closest(".glass-panel, .shop-item, button, a")) return;
      ...
      const newFood: FoodParticle[] = Array.from({ length: particles }, (_, i) => ({
        id: Math.random().toString(36).slice(2, 9),
